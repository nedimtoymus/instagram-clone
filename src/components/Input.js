...
export default function Input({label, type = 'text', ...props}) {
	...
	return (
		<label className="relative flex block border rounded-sm bg-zinc-50 focus-within:border-gray-400">
			...
			{type === 'password' && props?.value && (
				<button type="button" onClick={() => setShow(show => !show)} className="flex items-center h-full pr-2 text-sm font-semibold">
					{show ? 'Hide' : 'Show'}
				</button>
			)}
		</label>
